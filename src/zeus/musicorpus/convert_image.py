...
def convert_image(
    ...
    # `re_crop` and `normalize_image_height` are not implemented yet, and are
    # refused by the `musicorpus` command before any conversion starts. Asserted
    # rather than ignored, so that a second caller cannot quietly get neither.
    assert not re_crop, "Re-cropping is not yet implemented"
    assert normalize_image_height is None, "Image height normalization is not yet implemented"

    # locate the input image file
    input_image_path = mc_sample.musicorpus_path / "image.jpg"
    assert input_image_path.exists(), f"There is a missing image file at {input_image_path}"
    ...
    output_sample_path = output_path / Path(PosixPath(mc_sample.get_zeus_sample_name()))
