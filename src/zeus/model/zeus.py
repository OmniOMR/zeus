...
class Zeus:
    ...
    def visualize_training_data(
        ...
        for batch_index, (batch_images, batch_annotations) in enumerate(train_tf_dataset):
            ...
            for i in range(batch_images.shape[0]):
                ...
                lmx = self.token_map.indices_to_lmx(batch_annotations[i].numpy())
                html += "<div>"
                html += (
                    f'<img src="images/{sample_index}.png" '
                    f'heigt="{self.architecture_options.height}">'
                )
    ...
    def visualize_predictions(
        ...
        for sample_index, image, gold_lmx, pred_lmx, ser in items:
            ...
            html += "<div>"
            html += (
                f'<img src="{images_folder_path.name}/{sample_index}.jpg" '
                f'heigt="{self.architecture_options.height}">'
            )
