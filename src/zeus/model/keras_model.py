...
class KerasModel(tf.keras.Model):
    def __init__(self, architecture_options: ArchitectureOptions, token_map: TokenMap) -> None:
        super().__init__()
        ...
        inputs = tf.keras.layers.Input(
            shape=[self.architecture_options.height, None, 1], dtype=tf.float32, ragged=True
        )

        hidden = inputs.to_tensor()
        hidden = tf.keras.layers.Conv2D(
            self.architecture_options.cnn_dim, 3, 1, "same", use_bias=False
        )(hidden)
        for i in range(self.architecture_options.cnn_stages):
            filters = min(
                self.architecture_options.rnn_dim, self.architecture_options.cnn_dim * (1 << i)
            )
        ...
        hidden = tf.reshape(
            hidden, [tf.shape(hidden)[0], tf.shape(hidden)[1], hidden.shape[2] * hidden.shape[3]]
        )
        ...
        if remaining < 1:
            raise ValueError(
                "Inconsistent settings of timestep width "
                f"{self.architecture_options.timestep_width} and cnn stages "
                f"{self.architecture_options.cnn_stages}"
            )
        if remaining > 1:
            ...
            hidden = tf.reshape(
                ...
                [
                    tf.shape(hidden)[0],
                    tf.shape(hidden)[1] // remaining,
                    hidden.shape[2] * remaining,
                ],
            )
        ...
        for layer in range(self.architecture_options.rnn_layers):
            ...
            rnn_layer = tf.keras.layers.LSTM(
                self.architecture_options.rnn_dim, return_sequences=True
            )
        ...
        self._target_embedding = tf.keras.layers.Embedding(
            len(self.token_map), self.architecture_options.rnn_dim
        )
        ...
        self._target_output_layer = tf.keras.layers.Dense(len(self.token_map))

    def prepare_for_training(self, training_options: TrainingOptions, training_batch_count: int):
        ...
        if training_options.lr_decay == "cos":
            lr = tf.optimizers.schedules.CosineDecay(
                lr, training_batch_count * training_options.epochs
            )
        elif training_options.lr_decay != "none":
            raise ValueError(f"Unknown decay '{training_options.lr_decay}'")
    ...
    def decoder_training(self, encoded: tf.Tensor, targets: tf.Tensor) -> tf.Tensor:
        ...
        decoder_input = tf.concat(
            [tf.fill([tf.shape(targets)[0], 1], self.BOS), targets[:, :-1]], axis=-1
        )
    ...
    def decoder_inference(self, encoded: tf.Tensor, max_length: tf.Tensor) -> tf.Tensor:
        ...
        while tf.math.logical_and(
            index < max_length, tf.math.reduce_any(result_lengths == max_length)
        ):
            ...
            result_lengths = tf.where(
                (predictions == self.EOS) & (result_lengths > index), index, result_lengths
            )
            inputs = predictions
            index += 1
    ...
    def train_step(self, data):
        ...
        x, y = data
        y = tf.concat([y, tf.fill([tf.shape(y)[0], 1], self.EOS)], axis=-1)[
            :, : self.training_options.max_training_length
        ]
    ...
    def predict_step(self, data):
        ...
        y_pred = self.decoder_inference(encoded, self.inference_options.max_prediction_length)
        return y_pred
